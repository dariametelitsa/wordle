    ...
    return (
        <div className={'flex flex-wrap max-w-xl justify-center gap-1'}>
            {letters && letters.map(l => {
                ...
                return (<div className={`flex justify-center items-center w-12 h-16 border-2 rounded-2xl border-gray-100 transition ease-in-out delay-300 ${letterColor}`} key={l.key}>{l.key}</div>)
            })}
        </div>
    );
};